...
const UploadSchema = z.object({
  document_type: z.enum(['insurance', 'registration', 'mot_certificate', 'service_history', 'purchase_invoice', 'other']),
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (uploadError) {
    const msg = uploadError.message.toLowerCase().includes('bucket')
      ? 'Storage bucket "vehicle-documents" not found. Create it in Supabase Storage first.'
