...
export function Header() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 border-b border-border bg-white/95 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        <Logo className="animate-logo-reveal" />
      ...
      <div
        id="mobile-nav"
        className={cn(
          'border-t border-border bg-white transition-all duration-200 md:hidden',
          ...
          {navLinks.map((l) => (
            <Link
              ...
              className={cn(
                ...
                pathname === l.href ? 'text-gold' : 'text-muted hover:text-gray-900'
              )}
            >
              {l.label}
            </Link>
